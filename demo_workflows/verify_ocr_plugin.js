...
function workflow() {
  ...
  try {
    ...
    for (i = 0; i < testFiles.length; i++) {
      ...
      console.log(
        "[" + (i + 1) + "/" + testFiles.length + "] Processing: " + filename,
      );
      console.log("");
      ...
      var systemPrompt =
        "You are a helpful assistant that analyzes invoice and receipt documents. Provide a clear, concise summary in Japanese.";

      var userPrompt =
        "以下のPDFから抽出されたテキストを分析し、ドキュメントの内容を日本語で説明してください。" +
